...
const CONCEPT_PATTERNS: Record<SeraEvidenceConcept, RegExp[]> = {
  ...
  inadequateAssessment: [
    ...
    /\b(neither pilot|no pilot)\b.*\b(notic\w*|perceiv\w*|recogniz\w*|process\w*)\b/i,
    /\b(did not|failed to|n[aã]o|falhou em)\b.*\b(process\w*|processou|processar)\b.*\b(comment|callout|information|cue|coment[aá]rio|chamada|informa[cç][aã]o|sinal)\b/i,
    ...
    /\bn[aã]o (?:conseguia|conseguiu|podia)\b.*\bdistinguir\b.*\b(unidade|plataforma|pista|helideck|unit-[a-z0-9-]+|destino)\b/i,
    ...
    /\b(confundiu|confundiram|misidentified|mistook|equ[ií]voco)\b.*\b(unidade|plataforma|pista|destino|helideck|runway|surface|destination|deck)\b/i,
    /\bidentifica[cç][aã]o (incorreta|equivocada|errada)\b/i,
    /\b(passou a tratar|tratou|tratava|tomou)\b.*\b(unidade|plataforma|pista|destino|helideck|runway|surface|destination|deck)\b.*\b(como|por)\b.*\b(previst[ao]|planejad[ao]|corret[ao]|destino|pouso)\b/i,
    /\b(passou a tratar|tratou|tratava|tomou)\b.*\b(como|por)\b.*\b(unidade|plataforma|pista|destino|helideck|runway|surface|destination|deck)\b/i,
    /\bpassou a trat[aá]-l[oa]\b.*\bcomo\b.*\b(unidade|plataforma|pista|destino|helideck)\b/i,
    /\b(identificou|reconheceu|interpretou|interpretaram|confundiu|tomou)\b.*\b(unit-[a-z0-9-]+|pcp-?[0-9]+|unidade|plataforma|pista|destino|helideck)\b.*\b(como se fosse|como|por|pela|pelo)\b.*\b(unit-[a-z0-9-]+|pcp-?[0-9]+|unidade|plataforma|pista|destino|helideck)\b/i,
    /\b(acreditando|acreditou)\b.*\b(tratar-se|ser)\b.*\b(destino|unidade|plataforma|pista|helideck)\b/i,
    /\btomou\b.*\b(por|pela|pelo|como)\b.*\b(unidade|plataforma|pista|destino|helideck|runway|surface|destination|deck)\b/i,
    /\b(identificou|entendeu|acreditou|assumiu)\b.*\b(como|seria)\b.*\b(primeiro pouso|destino|unidade|plataforma|pista|helideck)\b/i,
  ...
  sensoryLimitation: [
    ...
    /\b(baixa visibilidade|visibilidade reduzida|visibilidade degradada|visual degradad[ao]|pistas? visuais? (?:perdid[ao]s?|ausentes?|degradad[ao]s?|limitad[ao]s?)|ambiente visual (?:degradad[ao]|ruim|limitad[ao]))\b/i,
    /\b(horizon|visual reference|visual references|sea surface|surface)\b.*\b(hard(?:er)? to distinguish|difficult to distinguish|lost|absent|missing|poor|dark|indistinct|confus\w*|obscur\w*)\b/i,
    ...
    /\brefer[eê]ncias? visuais?\b.*\b(degradad[ao]s?|limitad[ao]s?|insuficientes?|encobert[ao]s?)\b/i,
    /\b(chuva|n[eé]voa|nevoeiro|fog|rain)\b.*\b(encobria|encobriam|obscured|ocultava|ocultavam)\b.*\b(marca[cç][oõ]es|refer[eê]ncias|unidades?|plataformas?)\b/i,

    /\b(could not see|couldn't see|unable to see|visually impaired|blind|obscured)\b/i,
    /\b(n[aã]o conseguiu ver|n[aã]o podia ver|incapaz de ver|vis[aã]o prejudicada|encobert[ao])\b/i,
  ],
  knowledgeLimitation: [
    /\b(unfamiliar|knowledge gap|training gap|competence gap|not trained|lack of knowledge|lack of training|insufficient training)\b/i,
    /\b(n[aã]o familiar|lacuna de conhecimento|lacuna de treinamento|lacuna de compet[eê]ncia|n[aã]o treinad[ao]|desconhecia|falta de conhecimento|falta de treinamento|treinamento insuficiente)\b/i,
  ...
  perceptionCapabilityPresent: [
    ...
    /\b(coordenadas?)\b.*\b(inseridas?|programadas?)\b.*\b(normalmente|corretamente|correctly)\b/i,
    /\bgps\b.*\b(coordenadas?|destino|rota)\b.*\b(corret[ao]s?|programad[ao]s?|configurad[ao]s?|accurate|correct)\b/i,
    /\bcoordenadas? (?:estavam|eram|permaneciam)?\s*(?:corret[ao]s?|exat[ao]s?|precis[ao]s?)\b/i,
    /\b(coordenadas?|autoriza[cç][aã]o)\b.*\b(estavam|eram|permaneciam|continuavam)\b.*\b(corret[ao]s?|v[aá]lid[ao]s?|exat[ao]s?|precis[ao]s?)\b/i,
    /\b(ajuste|configura[cç][aã]o|programa[cç][aã]o)\b.*\bgps\b/i,
    /\b(gps|fms|sistema de navega[cç][aã]o|navega[cç][aã]o)\b.*\b(corretamente|adequadamente|corret[ao]s?)\b.*\b(configurad[ao]|programad[ao]|ajustad[ao]|coordenadas?|destino|rota)\b/i,
    /\b(gps|fms|sistema de navega[cç][aã]o|navega[cç][aã]o)\b.*\b(continha|continham|indicava|indicavam|mostrava|mostravam|apresentava|apresentavam)\b.*\b(coordenadas?|destino|rota|unit-|pcp)\b/i,

    /\b(visualizou|avistou|identified visually|visualized)\b.*\b(unidade|plataforma|helideck|guarni[cç][aã]o)\b/i,
    /\b(avistou|visualizou|viu|saw|visualized)\b.*\b(unit-[a-z0-9-]+|pcp-?[0-9]+|plataforma|helideck|destino)\b/i,
    /\b(recebeu|recebida|received|available to the crew)\b.*\b(mensagem|atualiza[cç][aã]o|informa[cç][aã]o|despacho|identificador)\b/i,
    /\b(mensagem|atualiza[cç][aã]o|informa[cç][aã]o|despacho)\b.*\b(recebida|recebido|dispon[ií]vel|presented|available)\b/i,

    /\b(treinamentos?|habilita[cç][oõ]es?|certificados?)\b.*\b(em dia|v[aá]lid[oa]s?|current|valid)\b/i,
  ],
  attentionPressure: [
  ...
  informationAmbiguous: [
    ...
    /\b(mensagens?|fontes?|identificadores?|despacho|atualiza[cç][aã]o)\b.*\b(diferentes?|divergentes?|conflitantes?|contradit[oó]ri[ao]s?)\b/i,
    /\b(despacho|mensagem|fonte)\b.*\bindicava\b.*\benquanto\b.*\b(atualiza[cç][aã]o|mensagem|outra fonte)\b.*\bindicava\b/i,
    /\bsem confirma[cç][aã]o\b.*\b(qual|qual delas|qual deles|substitu[ií]a|prevalecia|era v[aá]lida)\b/i,
  ...
  informationAvailableCorrect: [
    ...
    /\b(rota prevista|destino previsto|planned route|planned destination)\b/i,
    /\bautoriza[cç][aã]o\b.*\b(somente|apenas|only)\b/i,
    /\bautoriza[cç][aã]o\b.*\b(era|estava|permanecia|continuava|v[aá]lida)\b.*\b(para|unit-|pcp|destino|unidade)\b/i,
    /\b(primeiro destino|destino inicial|destino planejado|destino programado)\b.*\b(era|estava|previst[ao]|programad[ao])\b/i,
    /\bdestino\s+inicial\s+(?:previst[ao]|planejad[ao]|programad[ao])(?:\s+e\s+autorizad[ao])?\s+(?:era|foi)\b/i,
    /\b(plano|planejamento|rota)\b.*\b(previa|prev[eê]|indicava|definia|estabelecia)\b.*\b(destino|unit-|pcp|unidade|plataforma)\b/i,
    /\bdados?\b.*\b(corret[ao]s?|exat[ao]s?|precis[ao]s?)\b.*\b(gps|fms|sistema de navega[cç][aã]o|navega[cç][aã]o)\b/i,
    /\b(gps|fms|sistema de navega[cç][aã]o|navega[cç][aã]o)\b.*\bdados?\b.*\b(corret[ao]s?|exat[ao]s?|precis[ao]s?)\b/i,

    /\b(gps|fms|sistema de navega[cç][aã]o|navega[cç][aã]o)\b.*\b(corretamente|corret[ao]s?|adequadamente)\b.*\b(configurad[ao]|programad[ao]|coordenadas?|destino|rota)\b/i,

    /\b(display|mode display|mode annunciation|fma|visor|painel)\b.*\b(showed|displayed|indicated|mostrou|indicou|apresentou|mudou|alterou|change|mudan[cç]a|altera[cç][aã]o)\b/i,
    /\b(n[aã]o notou|n[aã]o percebeu|did not notice|failed to notice)\b.*\b(display|mode|modo|visor|painel|fma|indica[cç][aã]o)\b/i,
    /\bdever[ií]amos passar por cima\b.*\bdestino\b/i,
  ],
  informationUnavailable: [
    ...
    /\b(without (?:a )?clear annunciation|no clear annunciation|without alerting|sem (?:uma )?(?:indica[cç][aã]o|anuncia[cç][aã]o|sinaliza[cç][aã]o) clara|sem alertar)\b/i,
    /\b(voz baixa|low voice|quietly)\b.*\b(n[aã]o processou|did not process|failed to process|n[aã]o foi processad[ao])\b/i,
    /\b(mencionou|said|called out|comentou)\b.*\b(voz baixa|quietly|low voice)\b.*\b(n[aã]o processou|did not process|failed to process)\b/i,
  ...
  safeGoal: [
    ...
    /\bautoriza[cç][aã]o\b.*\b(somente|apenas|only)\b.*\b(destino|pcp|unit-[a-z0-9-]+|unidade|plataforma|pista|helideck)\b/i,
    /\bpassou a trat[aá]-l[oa]\b.*\bcomo\b.*\b(unidade|plataforma|pista|destino|helideck)\b.*\b(previst[ao]|planejad[ao]|programad[ao]|autorizad[ao])\b/i,
  ...
  knownRule: [
    ...
    /\b(o manual|o procedimento|a norma|o regulamento|a regra|a diretriz|o SOP)\b.*\b(exig\w*|determin\w*|estabelec\w*|define|requer|obriga|imp[eõ]e|prescreve|prev[eê])\b/i,
    ...
    /\b(?:the )?(?:departure |operating |approach |landing |takeoff )?procedure\b.*\b(stated|required|mandated|specified|called for|demanded|prescribed)\b/i,
  ...
  explicitAwareness: [
    ...
    /\b(mentioned|stated|said|noted|commented|remarked|pointed out)(?: to [^,.]{1,60})? that\b.*\b(below|above|outside|beyond|did not meet|was not|were not)\b/i,
  ],
  consciousDeviation: [
    ...
    // Natural PT expressions
    /\b(decidiu|optou|escolheu|resolveu|determinou|deliberou)\b.*\b(continuar|prosseguir|seguir|avan[cç]ar|proceder|manter|tentar|fazer|realizar|executar|partir|decolar)\b.*\b((mesmo|ainda que|embora|apesar de|a despeito de)|(sem|contra|violando|descumprindo|ignorando|desrespeitando))\b/i,
    /\b(decidiu|optou|escolheu|resolveu) (n[aã]o|nem)\b.*\b(interromper|abortar|cancelar|suspender|arremeter|descontinuar)\b/i,
    ...
    /\b(decided|chose|opted|resolved|elected) to\b.*\b(continue|proceed|take off|depart|descend|land|press on|carry on)\b/i,
    /\b(decidiu|optou|escolheu|resolveu)\b.*\b(continuar|prosseguir|decolar|partir|descer|pousar|manter a aproxima[cç][aã]o)\b/i,
    /\b(decidiram|optaram|escolheram|resolveram)\b.*\b(continuar|prosseguir|decolar|partir|descer|pousar|manter a aproxima[cç][aã]o)\b/i,
  ],
  routineDeviation: [
  ...
  unmanagedRisk: [
    /\b(contractual obligation|client pressure|client was becoming impatient|daily rotations?|productivity target|commercial pressure)\b/i,
    /\b(obriga[cç][aã]o contratual|press[aã]o do cliente|cliente (?:estava )?(?:impaciente|insatisfeito)|metas? di[aá]rias?|meta de voos?|press[aã]o comercial|continuidade da campanha)\b/i,
  ...
  incorrectAction: [
    /\b(pulled|pushed)\b.*\b(control column|column|stick|cyclic|collective)\b.*\b(instead of|rather than)\b/i,
    ...
    /\b(n[aã]o|falhou em|omitiu)\b.*\b(iniciar|executar|realizar)\b.*\b(arremetida|a[cç][aã]o exigida|procedimento)\b/i,
  ],
  physicalActionLimitation: [
  ...
  actionKnowledgeLimitation: [
    /\b(training gap|skill gap|knowledge gap|lack of (?:training|skill|knowledge)|insufficient (?:training|skill)|unfamiliarity|did not know how|not trained|unqualified)\b/i,
    /\b(lacuna de (?:treinamento|habilidade|conhecimento)|falta de (?:treinamento|habilidade|conhecimento)|treinamento insuficiente|habilidade insuficiente|n[aã]o familiaridade|n[aã]o sabia como|n[aã]o treinad[ao]|n[aã]o qualificad[ao])\b/i,
  ...
  actionCapabilityPresent: [
    ...
    /\b(captain|comandante|first officer|copiloto)\b.*\b(pilot flying|\bpf\b|operava os sistemas|operating the systems)\b/i,
    /\b(pilot flying|\bpf\b)\b.*\b(captain|comandante|first officer|copiloto)\b/i,
  ...
  selectionSubtype: [
    ...
    /\b(n[aã]o|deixou de|falhou em|omitiu)\b.*\b(iniciar|executar|realizar|selecionar|ajustar|configurar|inserir)\b.*\b(arremetida|procedimento|modo|altitude|n[ií]vel|valor)\b/i,
    /\b(inseriu|programou|selecionou|ajustou)\b.*\b(diferente|divergente|distinto)\b.*\b(carta|planejado|previsto|autorizado|briefing)\b/i,
    /\b(neither pilot|no pilot)\b.*\b(called for|called|initiated|ordered|requested)\b.*\b(go-around|go around)\b/i,
    /\b(captain|first officer|crew)\b.*\b(did not|failed to|declined to|omitted to)\b.*\b(call for|call|initiate|order|request)\b.*\b(go-around|go around)\b/i,
    /\b(nenhum dos pilotos|nenhum piloto)\b.*\b(chamou|comandou|iniciou|determinou|solicitou)\b.*\b(arremetida|go-around)\b/i,
    /\b(comandante|copiloto|tripula[cç][aã]o)\b.*\b(n[aã]o|deixou de|falhou em|omitiu)\b.*\b(chamar|comandar|iniciar|determinar|solicitar)\b.*\b(arremetida|go-around)\b/i,
  ],
  feedbackSubtype: [
