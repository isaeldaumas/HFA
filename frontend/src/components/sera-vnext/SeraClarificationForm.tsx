...
export function SeraClarificationForm(props: {
  ...
  async function submit() {
    ...
    if (!clarificationResponses.length) {
      setError('Preencha pelo menos uma resposta com informação factual do evento.')
      return
    }
    setState('loading')
    setError(null)
  ...
  return (
    <section className="rounded-xl border border-amber-500/40 bg-amber-950/20 p-5 space-y-4">
      <div>
        <h2 className="text-base font-semibold text-amber-200">Informações adicionais necessárias</h2>
      ...
      {questions.map((question, index) => (
        <div key={question.id} className="rounded-lg border border-amber-700/40 bg-slate-950/50 p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-amber-400">Pergunta {index + 1}</p>
          ...
          <textarea
            ...
            placeholder="Descreva a evidência factual disponível."
          />
        </div>
      ))}
      ...
      >
        {state === 'loading' ? 'Reanalisando…' : 'Adicionar evidência e reanalisar'}
      </button>
    </section>
  )
}
