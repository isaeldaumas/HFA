...
import { isAllowedA4R190LegacyMethodologyPath, isAllowedSeraVNextCanonicalTreePath, isAllowedSeraVNextEngineV03CalibrationPath } from './protected-path-contract'
...
for (const file of changed) {
  for (const prefix of protectedPrefixes) {
    if (file.startsWith(prefix) && (isAllowedSeraVNextCanonicalTreePath(rootDir, file) || isAllowedSeraVNextEngineV03CalibrationPath(rootDir, file) || isAllowedA4R190LegacyMethodologyPath(rootDir, file))) continue
