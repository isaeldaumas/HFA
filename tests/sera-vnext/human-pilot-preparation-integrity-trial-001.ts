...
import { isAllowedA4R190LegacyMethodologyPath, isAllowedPrimarySeraMigrationPath, isAllowedSeraVNextCanonicalTreePath, isAllowedSeraVNextEngineV03CalibrationPath } from './protected-path-contract'
...
for (const file of changed) {
  for (const prefix of protectedPrefixes) {
    if (file.startsWith(prefix) && (isAllowedSeraVNextCanonicalTreePath(rootDir, file) || isAllowedSeraVNextEngineV03CalibrationPath(rootDir, file) || isAllowedPrimarySeraMigrationPath(rootDir, file) || isAllowedA4R190LegacyMethodologyPath(rootDir, file))) continue
