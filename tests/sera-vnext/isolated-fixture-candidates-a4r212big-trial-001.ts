...
for (const changedPath of changedTracked) {
  ...
  assert.ok(
    isAllowedA4R190LegacyMethodologyPath(root, changedPath) || isAllowedPrimarySeraMigrationPath(root, changedPath) || !protectedPrefixes.some((prefix) => changedPath.startsWith(prefix)),
